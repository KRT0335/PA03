...
isNegate:			@CHECK IF NEGATE
	push	{r1-r2, lr}	@adds space
	mov	r1, r0		@sets r1 to be r0
	ldrb 	r2, [r1]	@load the first byte of the string r1

	cmp 	r2, #0x2D	@check if the first byte is '-'
	beq	endIsNegate	@if '-' goto endIsNegate

	mov 	r0, #0		@r0 = 0 meaning the number is NOT negative
	pop	{r1-r2, lr}	@removes space
	bx lr			@returning r0
endIsNegate:
	mov 	r0, #1		@r0 = 1 meaning the number is negative
	pop	{r1-r2, lr}	@removes space
	bx lr			@returning r0

@ assumes r0 the address of the first byte of string
@ returns with r0 == 1 if it was a valid operation ('+', '-', '*', '/'), else 0
isOperation:			@CHECK IF OPERATION
	push	{r1-r3, lr}	@adds space
	mov	r1, r0		@sets r1 to be r0
	ldrb 	r2, [r1]	@load the first byte of the string r1

	cmp	r2, #0x2B	@check if the byte is '+'
	ldreqb 	r2, [r1, #1]	@load the second byte of the string r1
	cmpeq 	r2, #0x00	@checks if '\0'
	beq 	endIsOperation	@if '\0' goto endIsOperation ending this function

	cmp 	r2, #0x2D	@check if the byte is '-'
	ldreqb 	r2, [r1, #1]	@load the second byte of the string r1
	cmpeq	r2, #0x00	@checks if '\0'
	beq	endIsOperation	@if '\0' goto endIsOperation ending this function

	cmp	r2, #0x2A	@check if the byte is '*'
	ldreqb 	r2, [r1, #1]	@load the second byte of the string r1
	cmpeq	r2, #0x00	@checks if '\0'
	beq	endIsOperation	@if '\0' goto endIsOperation ending this function

	cmp 	r2, #0x2F	@check if the byte is '/'
	ldreqb 	r2, [r1, #1]	@load the second byte of the string r1
	cmpeq	r2, #0x00	@checks if '\0'
	beq	endIsOperation	@if '\0' goto endIsOperation ending this function

	mov 	r0, #0		@r0 = 0 meaning the input is NOT an operation
	pop	{r1-r3, lr}	@removes space
	bx lr			@returning r0
endIsOperation:
	mov 	r0, #1		@r0 = 1 meaning the input is an operation
	pop	{r1-r3, lr}	@removes space
	bx lr			@returning r0
...
isNumber:			@CHECK IF NUMBER
	push	{r1-r8, lr}	@adds space
	mov	r1, r0		@sets r1 to be r0
	ldrb	r2, [r1]	@load the first byte of the string r1
	mov 	r3, #1		@r3 values as the counter to point to the next byte in string
	cmp	r2, #0x2D	@checks if number if negative
	ldreqb	r2, [r1, r3]	@if negative move to next byte along the string
	addeq	r3, r3, #1	@moves the counter for the next byte
loopIsNumber:			@loops until the string is all numbers and reach '\0' or non-integer
	cmp	r2, #0x30	@checks if r2 < 0
	blt	endIsNumber	@if true, end the loop, not a number
	cmp	r2, #0x39	@checks if r2 > 9
	bgt	endIsNumber	@if true, end the loop, not a number
	ldrb	r2, [r1, r3]	@load r2 to the next byte along the r1 string
	cmp	r2, #0x00	@checks if '\0'
	beq	endIsNumberGood	@if true, end of the string, state this string is a number
	add	r3, r3, #1	@if not, add 1 to r3 to move on to the next byte in the string
	b	loopIsNumber	@also loop through loopIsNumber, checking the next byte
endIsNumberGood:		@met when the string is confirmed to be a number
	mov	r0, #1		@return r0 = 1 meaning the string is a number
	pop	{r1-r8, lr}	@removes space
	bx lr			@ends function and return r0
endIsNumber:			@met if a byte in the string is not a number except '\0'
	mov	r0, #0		@return r0 = 0 meaning the string is NOT a number
	pop	{r1-r8, lr}	@removes space
	bx lr			@ends function and return r0
...
executeAdd:			@ADD
	push	{r2, lr}	@adds space
	adds 	r2, r0, r1	@r0 + r1 into r2 as a place holder
	bvs	handleOverflow	@checks for overflow
	mov	r0, r2		@if not overload, return r0 with r2's value
	pop	{r2, lr}	@removes space
	bx lr			@ends function and return r0
	
@ executed on binary '-' operation
executeSubtract:		@SUB
	push	{r2, lr}	@adds space
	subs 	r2, r0, r1	@r0 - r1 into r2 as a place holder
	bvs	handleOverflow	@checks for overflow
	mov	r0, r2		@if not overload, return r0 with r2's value
	pop	{r2, lr}	@removes space
	bx lr			@ends function and return r0
...
@ executed on binary '*' operation
executeMultiply:			@MULTIPLY
	push	{r2, r3, r4, lr}	@adds space
	smull 	r2, r3, r0, r1		@multiply r0*r1 with results in the range of [r2,r3]
	cmp 	r3, r2, asr #31		@cmp the range with a shift of 31 bits to test for overload
	blne	handleOverflow		@if not equal, then overflow occured
					@after testing for overload it is safe to multiply
	mov	r4, r0			@have r4 = r0 to avoid error when compile
	muls	r0, r4, r1		@r0 = r4*r1 = r0*r1, multi. occurs
	pop	{r2, r3, r4, lr}	@removes space
	bx lr				@ends function and return r0
	
@ executed on binary '/' operation (hint, use div function below)
executeDivide:				@DIVISION
	push	{r2, r3, r4, lr}	@adds space
	cmp	r1, #0			@check if the denominator is zero
	bleq	handleDivByZero		@if true, links handleDivByZero to handle error
	blne	div			@if not, execute the division operation
	bvs	handleOverflow		@if overload, branches to handleOverflow
	pop	{r2, r3, r4, lr}	@removes space
	bx lr				@ends function and return r0, the result rounded down
	
@ executed on unary (one input) '-' operation
executeNegate:			@NEGATE	
	push	{lr}		@adds space
	mvn	r0, r0		@makes the input negative
	bvs	handleOverflow	@just in case checks for overflow
	pop	{lr}		@removes space
	bx lr			@ends function and return -r0
...
handleUndefined:			@Undefined
	push	{r0, r1, lr}		@adds space
	ldr	r0, =ADDR_UART0		@r0:=ADDR_UART0
	ldr	r1, =string_undefined	@r1 contains error message
	bl 	print_string		@branch links to print error message
	mov	r8, #1			@flag and error has occures with r8 = 1
	pop	{r0, r1, lr}		@removes space
	bx lr				@breaks origin function and return prior
...
handleOverflow:				@OVERFLOW
	push	{r0, r1, lr}		@adds space
	ldr	r0, =ADDR_UART0		@r0:=ADDR_UART0
	ldr	r1, =string_overflow	@r1 contains error message
	bl 	print_string		@branch links to print error message
	mov	r8, #1			@flag and error has occures with r8 = 1
	pop	{r0, r1, lr}		@removes space
	bx lr				@breaks origin function and return prior
...
@ should print appropriate error message
handleDivByZero:			@DivByZero
	push	{r0, r1, lr}		@adds space
	ldr	r0, =ADDR_UART0		@r0:=ADDR_UART0
	ldr	r1, =string_div_by_zero	@r1 contains error message
	bl 	print_string		@branch links to print error message
	mov	r8, #1			@flag and error has occures with r8 = 1
	pop	{r0, r1, lr}		@removes space
	bx lr				@breaks origin function and return prior
